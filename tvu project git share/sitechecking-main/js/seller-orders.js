  ...
  if (container) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <button onclick="openGlobalAuthModal('signup')" class="btn btn-primary" style="margin-top:1rem;">Sign in / Register</button>
      </div>
    `;
  }
}
...
function renderSellerOrders() {
  ...
  container.innerHTML = sellerOrders.map((order) => {
    ...
    if (isDelivered) {
      badgeHtml = `<span class="badge badge-stock-in">Delivered / Completed</span>`;
      actionHtml = `<span class="badge badge-stock-in" style="font-size:0.85rem; padding:0.4rem 0.8rem;">Completed</span>`;
    } else if (isConfirmed) {
      const dateText = formatDeliveryDate(order.deliveryDate);
      badgeHtml = `<span class="badge badge-category">Order Confirmed (Delivery: ${escapeHTML(dateText)})</span>`;
      actionHtml = `
      ...
      badgeHtml = `<span class="badge badge-discount">Waiting for Confirmation</span>`;
      actionHtml = `
